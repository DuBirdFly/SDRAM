...
module sdram_model_plus #(
    parameter addr_bits =	13,         // 2^13 = 8192, ROW Address
    parameter col_bits  =	9,          // 2^9  = 512,  COL Address
    ...
    input                           Cas_n,
    input                           We_n,
    input                 [3 : 0]   Dqm,          // [3:2] 位无效
    input			                Debug         // added by xzli, 由内部信号转为端口信号
);

localparam mem_sizes = (2**addr_bits) * (2**col_bits) - 1;

    // Timing Parameters: -75 (PC133) and CAS Latency = 2
    /*
    parameter tAC  =   8;
    parameter tHZ  =   7.0;
    parameter tOH  =   2.7;
    ...
    */
    ...
    // Timing Parameters: -7 (PC143) and CAS Latency = 3
    parameter tAC  =   6.5;
    parameter tHZ  =   5.5;
    parameter tOH  =   2;
    parameter tMRD =   2.0; // 2 Clk Cycles
    ...
    integer	f_sdram_log;
    initial begin
		// f_sdram_log = $fopen("f_sdram_log.txt");
	end
    ...
    always@(posedge Clk) cke_temp <= Cke;

    assign	sdram_control={Cs_n,Ras_n,Cas_n,We_n};

    always@(sdram_control or cke_temp) begin
        case(sdram_control)
            ...
            Write        : 
                if(cke_temp==1) sdram_command <= "Write";
                else            sdram_command <= "Write_suspend";
            Read         :
                if(cke_temp==1) sdram_command <= "Read";
                else            sdram_command <= "Read_suspend";
            Nop          :
                if(cke_temp==1) sdram_command <= "Nop";
                else            sdram_command <= "Self_refresh";
    ...
    always @ (posedge Sys_clk) begin
        ...
        if (Auto_precharge[0] == 1'b1) Count_precharge[0] = Count_precharge[0] + 1;
        if (Auto_precharge[1] == 1'b1) Count_precharge[1] = Count_precharge[1] + 1;
        if (Auto_precharge[2] == 1'b1) Count_precharge[2] = Count_precharge[2] + 1;
        if (Auto_precharge[3] == 1'b1) Count_precharge[3] = Count_precharge[3] + 1;
        ...
        if (Aref_enable == 1'b1) begin
            ...
            if ($time - RC_chk < tRC)
                $display ("@%t, ERROR: violate tRC when AREF (AREF to AREF)", $time);
            if ($time - RP_chk0 < tRP || $time - RP_chk1 < tRP || $time - RP_chk2 < tRP || $time - RP_chk3 < tRP)
                $display ("@%t, ERROR: violate tRP when AREF (PRE to AREF)", $time);
            if (Pc_b0 == 1'b0 || Pc_b1 == 1'b0 || Pc_b2 == 1'b0 || Pc_b3 == 1'b0)
                $display ("@%t, ERROR: All-banks must PRE before AREF", $time);
            RC_chk = $time; // Record Current tRC time
        end
        ...
        if (Mode_reg_enable == 1'b1) begin
            ...
            if (Pc_b0 == 1'b1 && Pc_b1 == 1'b1 && Pc_b2 == 1'b1 && Pc_b3 == 1'b1) begin
                ...
                if (Debug) begin
                    $display ("@%t, LMR", $time);
                    // CAS Latency
                    if      (Addr[6 : 4] == 3'b010)  $display ("\tCAS Latency = 2");
                    else if (Addr[6 : 4] == 3'b011)  $display ("\tCAS Latency = 3");
                    else                             $display ("\tCAS Latency = Reserved");
                    // Burst Length
                    if      (Addr[2 : 0] == 3'b000)  $display ("\tBurst Length = 1");
                    else if (Addr[2 : 0] == 3'b001)  $display ("\tBurst Length = 2");
                    else if (Addr[2 : 0] == 3'b010)  $display ("\tBurst Length = 4");
                    else if (Addr[2 : 0] == 3'b011)  $display ("\tBurst Length = 8");
                    else if (Addr[3 : 0] == 4'b0111) $display ("\tBurst Length = Full");
                    else                             $display ("\tBurst Length = Reserved");
                    // Burst Type
                    if      (Addr[3] == 1'b0)        $display ("\tBurst Type = Sequential");
                    else if (Addr[3] == 1'b1)        $display ("\tBurst Type = Interleaved");
                    else                             $display ("\tBurst Type = Reserved");
                    // Write Burst Mode
                    if      (Addr[9] == 1'b0)        $display ("\tWrite Burst Mode = Programmed Burst Length");
                    else if (Addr[9] == 1'b1)        $display ("\tWrite Burst Mode = Single Location Access");
                    else                             $display ("\tWrite Burst Mode = Reserved");
                end
            end
            else $display ("@%t, ERROR: all banks must be PRE before LMR", $time);

            if ($time - RC_chk < tRC) $display ("@%t, ERROR: violate tRC when LMR (REF -> LMR)", $time);
            if (MRD_chk < tMRD)       $display ("@%t, ERROR: violate tMRD when LMR (LMR -> LMR)", $time);
        ...
        if (Active_enable == 1'b1) begin
            ...
            if (Debug) $display ("@%t, ACT: Bank = %0d, Row = %0d", $time, Ba, Addr);
            ...
            case (Ba)
                2'b00: begin
                    if (Pc_b0 == 1'b1) begin
                        ...
                        RAS_chk0 = $time;
                        if ($time - RP_chk0 < tRP)
                            $display ("@%t, ERROR: violate tRP when Activate bank-0 (PRE -> ACT)", $time);
                    end
                    else $display ("@%t, ERROR: Bank 0 is not Precharged before Active", $time);
                end
                2'b01: begin
                    if (Pc_b1 == 1'b1) begin
                        {Act_b1, Pc_b1} = 2'b10;
                        ...
                        RAS_chk1 = $time;
                        if ($time - RP_chk1 < tRP)
                            $display ("@%t, ERROR: violate tRP when Activate bank 1 (PRE -> ACT)", $time);
                    end
                    else $display ("@%t, ERROR: Bank 1 is not Precharged before Active", $time);
                end
                2'b10: begin
                    if (Pc_b2 == 1'b1) begin
                        {Act_b2, Pc_b2} = 2'b10;
                        ...
                        RAS_chk2 = $time;
                        if ($time - RP_chk2 < tRP)
                            $display ("@%t, ERROR: violate tRP when Activate bank 2 (PRE -> ACT)", $time);
                    end
                    else $display ("@%t, ERROR: Bank 2 is not Precharged before Active", $time);
                end
                2'b11: begin
                    if (Pc_b3 == 1'b1) begin
                        {Act_b3, Pc_b3} = 2'b10;
                        ...
                        RAS_chk3 = $time;
                        if ($time - RP_chk3 < tRP)
                            $display ("@%t, ERROR: violate tRP when Activate bank 3 (PRE -> ACT)", $time);
                    end
                    else $display ("@%t, ERROR: Bank 3 is not Precharged before Active", $time);
                end
            endcase

            if ((Previous_bank != Ba) && ($time - RRD_chk < tRRD))
                $display ("@%t, ERROR: violate tRRD when Activate bank-%0d (ACT -> ACT)", $time, Ba);

            if (MRD_chk < tMRD )
                $display ("@%t, ERROR: violate tMRD when Activate bank-%0d (LMR -> ACT)", $time, Ba);

            if ($time - RC_chk < tRC)
                $display ("@%t, ERROR: violate tRC  when Activate bank-%0d (AREF -> ACT)", $time, Ba);
        ...
        if (Prech_enable == 1'b1) begin
            if (Addr[10] == 1'b1) begin
                ...
                RP_chk2 = $time;
                RP_chk3 = $time;
                if (Debug) $display ("@%t, PRE (Bank-ALL)",$time);
                if (($time - RAS_chk0 < tRAS) || ($time - RAS_chk1 < tRAS) ||
                    ($time - RAS_chk2 < tRAS) || ($time - RAS_chk3 < tRAS)) begin
                    $display ("@%t, ERROR: violate tRAS when PRE-All-Banks (ACT -> PRE-All-Banks)", $time);
                end
                if (($time - WR_chk[0] < tWRp) || ($time - WR_chk[1] < tWRp) ||
                    ($time - WR_chk[2] < tWRp) || ($time - WR_chk[3] < tWRp)) begin
                    $display ("@%t, ERROR: violate tWR when PRE-All-Banks", $time);
                end
            end
            else if (Addr[10] == 1'b0) begin
                if (Debug) $display ("@%t, PRE (Bank-%0d)",$time, Ba);
                case (Ba)
                    2'b00: begin
                        {Pc_b0, Act_b0} = 2'b10;
                        RP_chk0 = $time;
                        if ($time - RAS_chk0 < tRAS)
                            $display ("@%t, ERROR: violate tRAS when Precharge bank 0 (ACT -> PRE)", $time);
                    end
                    2'b01: begin
                        {Pc_b1, Act_b1} = 2'b10;
                        RP_chk1 = $time;
                        if ($time - RAS_chk1 < tRAS)
                            $display ("@%t, ERROR: violate tRAS when Precharge bank 1 (ACT -> PRE)", $time);
                    end
                    2'b10: begin
                        {Pc_b2, Act_b2} = 2'b10;
                        RP_chk2 = $time;
                        if ($time - RAS_chk2 < tRAS)
                            $display ("@%t, ERROR: violate tRAS when Precharge bank 2 (ACT -> PRE)", $time);
                    end
                    2'b11: begin
                        {Pc_b3, Act_b3} = 2'b10;
                        RP_chk3 = $time;
                        if ($time - RAS_chk3 < tRAS)
                            $display ("@%t, ERROR: violate tRAS when Precharge bank 3 (ACT -> PRE)", $time);
                    end
                endcase


                // tWR violation check for write
                if ($time - WR_chk[Ba] < tWRp) $display ("@%t, ERROR: violate tWR when PRE-BANK-%0d", $time, Ba);
            end

            // Terminate a Write Immediately (if same bank or all banks)
            if (Data_in_enable == 1'b1 && (Bank == Ba || Addr[10] == 1'b1))
                Data_in_enable = 1'b0;
            ...
            else if (Cas_latency_2 == 1'b1) begin
                Command[1] = `PRECH;
                Bank_precharge[1] = Ba;
        ...
        if (Read_enable == 1'b1 || Write_enable == 1'b1) begin
            ...
            if ((Ba == 2'b00 && Pc_b0 == 1'b1) || (Ba == 2'b01 && Pc_b1 == 1'b1) ||
                ...
                $display("@%t, ERROR: Cannot Read or Write - Bank %d is not Activated", $time, Ba);
            end
            // Activate to Read or Write
            if ((Ba == 2'b00) && ($time - RCD_chk0 < tRCD))
                $display("@%t, ERROR: tRCD violation during Read or Write to Bank 0", $time);
            if ((Ba == 2'b01) && ($time - RCD_chk1 < tRCD))
                $display("@%t, ERROR: tRCD violation during Read or Write to Bank 1", $time);
            if ((Ba == 2'b10) && ($time - RCD_chk2 < tRCD))
                $display("@%t, ERROR: tRCD violation during Read or Write to Bank 2", $time);
            if ((Ba == 2'b11) && ($time - RCD_chk3 < tRCD))
                $display("@%t, ERROR: tRCD violation during Read or Write to Bank 3", $time);
            // Read Command
            if (Read_enable == 1'b1) begin
                // CAS Latency pipeline
                if      (Cas_latency_3 == 1'b1) begin
                    if (Addr[10] == 1'b1) Command[2] = `READ_A;
                    else                  Command[2] = `READ;
                    Col_addr[2] = Addr;
                    Bank_addr[2] = Ba;
                end
                else if (Cas_latency_2 == 1'b1) begin
                    if (Addr[10] == 1'b1) Command[1] = `READ_A;
                    else                  Command[1] = `READ;
                    Col_addr[1] = Addr;
                    Bank_addr[1] = Ba;
                end
                ...
                if (Data_in_enable == 1'b1) Data_in_enable = 1'b0;

            end
            // Write Command
            else if (Write_enable == 1'b1) begin
                if (Addr[10] == 1'b1) Command[0] = `WRITE_A;
                else                  Command[0] = `WRITE;
                Col_addr[0] = Addr;
                Bank_addr[0] = Ba;
                ...
                if (Data_out_enable == 1'b1) Data_out_enable = 1'b0;
            end

            // Interrupting a Write with Autoprecharge
            if (Auto_precharge[Bank] == 1'b1 && Write_precharge[Bank] == 1'b1) begin
                RW_interrupt_write[Bank] = 1'b1;
                if (Debug) $display ("@%t, NOTE : Read/Write Bank %d interrupt Write Bank %d with Autoprecharge", $time, Ba, Bank);
            end

            // Interrupting a Read with Autoprecharge
            if (Auto_precharge[Bank] == 1'b1 && Read_precharge[Bank] == 1'b1) begin
                RW_interrupt_read[Bank] = 1'b1;
                if (Debug) $display ("@%t, NOTE : Read/Write Bank %d interrupt Read Bank %d with Autoprecharge", $time, Ba, Bank);
            end
            ...
            if (Addr[10] == 1'b1) begin
                ...
                if      (Read_enable == 1'b1)  Read_precharge[Ba] = 1'b1;
                else if (Write_enable == 1'b1) Write_precharge[Ba] = 1'b1;
            end
        end
            ...
            if ((($time - RAS_chk0 >= tRAS) &&                                                      // Case 2
                ((Burst_length_1 == 1'b1 && Count_precharge[0] >= 1) ||                             // Case 1
                 ...
                 (RW_interrupt_read[0] == 1'b1)) begin                                              // Case 3
                    ...
                    if (Debug) $display ("@%t, NOTE : Start Internal Auto Precharge for Bank 0", $time);
            end
        end
            ...
            if ((($time - RAS_chk1 >= tRAS) &&
                ((Burst_length_1 == 1'b1 && Count_precharge[1] >= 1) ||
                 ...
                 (RW_interrupt_read[1] == 1'b1)) begin
                    ...
                    if (Debug) $display ("@%t, NOTE : Start Internal Auto Precharge for Bank 1", $time);
            end
        end
            ...
            if ((($time - RAS_chk2 >= tRAS) &&
                ((Burst_length_1 == 1'b1 && Count_precharge[2] >= 1) ||
                 ...
                 (RW_interrupt_read[2] == 1'b1)) begin
                    ...
                    if (Debug) $display ("@%t, NOTE : Start Internal Auto Precharge for Bank 2", $time);
            end
        end
            ...
            if ((($time - RAS_chk3 >= tRAS) &&
                ((Burst_length_1 == 1'b1 && Count_precharge[3] >= 1) ||
                 ...
                 (RW_interrupt_read[3] == 1'b1)) begin
                    ...
                    if (Debug) $display ("@%t, NOTE : Start Internal Auto Precharge for Bank 3", $time);
            end
        end
        ...
        if (Data_in_enable == 1'b1) begin                                   // Writing Data to Memory
           ...
           // if (Dqm[3] == 1'b0) Dq_dqm [31 : 24] = Dq [31 : 24];
            ...
            $display("@%t, WRITE: Bank = %d Row = %d, Col = %d, Data = Hi-Z due to DQM", $time, Bank, Row, Col);
            //$fdisplay(f_sdram_log,"bank:%h	row:%h	col:%h	write:%h",Bank,Row,Col,Dq_dqm);

            // Output result
            // if (Dqm == 4'b1111) begin
            if (Dqm[1:0] == 4'b11) begin
                if (Debug) $display("@%t, WRITE: Bank = %d Row = %d, Col = %d, Data = Hi-Z due to DQM", $time, Bank, Row, Col);
            end else begin
                if (Debug) $display("@%t, WRITE: Bank = %d Row = %d, Col = %d, Data = %d, Dqm = %b", $time, Bank, Row, Col, Dq_dqm, Dqm);
            ...
            if (Dqm_reg0 == 4'b1111) begin
                if (Debug) $display("@%t, READ : Bank = %d Row = %d, Col = %d, Data = Hi-Z due to DQM", $time, Bank, Row, Col);
            end else begin
                if (Debug) $display("@%t, READ : Bank = %d Row = %d, Col = %d, Data = %d, Dqm = %b", $time, Bank, Row, Col, Dq_reg, Dqm_reg0);
            end
            // Advance burst counter subroutine
            Burst;
        end
